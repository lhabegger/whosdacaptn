      ...
      <main class="container grid lg:grid-cols-5">
        ...
        <aside
                  ...
                  <div class="lg:col-span-4">
                            ...
                            <td>
                              ...
                              <div>
                                <input type="number" class="input input-bordered input-xs  max-w-xs" placeholder="custom score"
                                  v-model.number="player.customScore" />
                                <button class="btn btn-xs btn-primary"
                                  @click="modifyScore(player.name, player.customScore)">Go!</button>
                              </div>
                            </td>
                            <td>{{ player.score }}</td>
                          </tr>
                      ...
                      <div>
                        ...
                        <TransitionGroup name="fade" tag="ol">
                          <li v-for="(player, index) in sortedPlayers" :key="player.name" class="p-5">
                            ...
                            <img style="display:inline; margin-left:-40px; margin-right:10px;" v-if="index === 0"
                              src="/sailor-hat.png" alt="Icon" width="30" height="30" />

                            <span class="mr-3 font-bold text-xl" @click="showCallout(player.name)">{{ player.name }}</span><span>{{
                              player.score }}</span>
                          </li>
                        </TransitionGroup>
                    ...
                    <LeaderBoardActionsCallout :player="clickedPlayer" @modifyScore="modifyScore" @hideCallout="hideCallout" />
                    <div class="divider"></div>
                    <section class="container py-5">
                      <div>
                        ...
                        <form>
                          ...
                          <button type="button" class="btn" @click="addPlayer(name)">
                            Spieler hinzufügen
                          </button>
                        </form>
                    ...
                    <!-- Past Games -->
                    <section class="container py-5">
                      <h2 class="heading2">Vergangene Spiele</h2>
                      <table class="table">
                        <thead>
                          <tr>
                            <th>Game ID</th>
...
const showCallout = (playerName: string) => {
  // Set the clickedPlayer to the selected player's name
